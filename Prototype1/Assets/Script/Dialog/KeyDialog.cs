...
public class KeyDialog : MonoBehaviour
{
    ...
    [TextArea(15, 20)] public string keyInInventDialog;
    ...
    public void GetKeyDialog()
    {
        if (!dialogBox.activeInHierarchy && !isKeyInInventory)
        {
            isKeyInInventory = true;
            dialogBox.SetActive(true);
            dialogText.text = keyInInventDialog;
            Debug.Log("Key is now in Inventory...");
        }

        else if (dialogBox.activeInHierarchy && isKeyInInventory)
        {
            dialogBox.SetActive(false);
        }
    }
